import { r as db, N as eventRegistrations, m as eq, y as events, d as desc } from '../../../../chunks/db.js-CPhQsSSC.js';
...
import { o as object, n as number, _ as _enum } from '../../../../chunks/auth.js-CzaJ0OJC.js';
...
var actions = {
	status: async ({ request }) => {
		...
		try {
			const [result] = await db.update(eventRegistrations).set({ status: form.data.status }).where(eq(eventRegistrations.id, form.data.id));
			if (result.affectedRows === 0) return message(form, {
				type: "error",
				text: "That registration no longer exists"
			}, { status: 404 });
			return message(form, {
				type: "success",
	...
	delete: async ({ request }) => {
		...
		if (!form.valid) return fail$1(400, { form });
		try {
			const [result] = await db.delete(eventRegistrations).where(eq(eventRegistrations.id, form.data.id));
			if (result.affectedRows === 0) return message(form, {
				type: "error",
				text: "That registration was already deleted"
			}, { status: 404 });
			return message(form, {
				type: "success",
...
//# sourceMappingURL=_page.server.ts.js-j41FXr9g.js.map
