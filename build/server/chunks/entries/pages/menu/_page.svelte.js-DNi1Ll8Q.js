...
function Menu_item_card($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<div class="menu-row py-6 first:pt-0"><div class="mb-2 flex flex-wrap items-baseline gap-x-4 gap-y-1"><h4${attr_class(`font-heading min-w-0 text-lg tracking-[0.08em] uppercase ${item.isAvailable ? "text-foreground" : "text-muted-foreground line-through"}`)}>${escape_html(item.name)}</h4> <span class="menu-leader"></span> <span class="text-gold shrink-0 font-heading text-base tracking-[0.06em]">£${escape_html((item.price / 100).toFixed(2))}</span></div> `);
...
//# sourceMappingURL=_page.svelte.js-DNi1Ll8Q.js.map
