...
function Menu_item_card($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<div class="menu-row py-6 first:pt-0"><div class="mb-2 flex items-baseline gap-4"><h4${attr_class(`font-heading flex flex-wrap items-baseline gap-3 text-lg tracking-[0.08em] whitespace-nowrap uppercase ${item.isAvailable ? "text-cream" : "text-muted-foreground line-through"}`)}>${escape_html(item.name)}</h4> <span class="menu-leader"></span> <span class="text-gold shrink-0 font-heading text-base tracking-[0.06em]">£${escape_html((item.price / 100).toFixed(2))}</span></div> `);
...
//# sourceMappingURL=_page.svelte.js-BTp5cj5g.js.map
